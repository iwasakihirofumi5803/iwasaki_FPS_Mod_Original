...
public class EnemyMobile : MonoBehaviour
{
    ...
    [Tooltip("Fraction of the enemy's attack range at which it will stop moving towards target while attacking")] //攻撃中にターゲットへの移動を停止する敵の攻撃範囲の割合
    [Range(0f, 1f)]
    public float attackStopDistanceRatio = 0.5f;
    [Tooltip("The random hit damage effects")]  //ランダムヒットダメージ効果
    ...
    void UpdateCurrentAIState()
    {
        ...
        switch (aiState)
        {
            case AIState.Patrol:    //AI状態。パトロール
                m_EnemyController.UpdatePathDestination();
                m_EnemyController.SetNavDestination(m_EnemyController.GetDestinationOnPath());
                break;
            case AIState.Follow:    //AI状態。フォロー
                m_EnemyController.SetNavDestination(m_EnemyController.knownDetectedTarget.transform.position);
                m_EnemyController.OrientTowards(m_EnemyController.knownDetectedTarget.transform.position);
                m_EnemyController.OrientWeaponsTowards(m_EnemyController.knownDetectedTarget.transform.position);
                break;
            case AIState.Attack:    //AI状態。攻撃
